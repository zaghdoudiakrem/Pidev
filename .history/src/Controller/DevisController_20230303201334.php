    ...
    public function generatePdfAction($id)
{
   ...
   #[Route('/send-mail/{id}', name:'app_send_mail', methods: ['GET'])]
    ...
    //Get the mechanic's email
    $mechanicEmail = $devi->getMecanicien()->getEmail();
     //Get the expert's email
     $expertEmail = $devi->getExpert()->getEmail();

     //Set who the message is to be sent from
     $mail->setFrom($mechanicEmail, 'Mechanic Name');
 
     //Set who the message is to be sent to
     $mail->addAddress($expertEmail, 'Expert Name');
        //Set who the message is to be sent from
        $mail->setFrom('wadii.sdouga@esprit.tn', 'Your Name');

        //Set who the message is to be sent to
        $mail->addAddress('wadii.sdouga2000@gmail.com', 'Recipient Name');
