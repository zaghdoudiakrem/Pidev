...
class DevisController extends AbstractController
{
    ...
    public function updatedevisJSON(Request $req, $id, NormalizerInterface $Normalizer)
    {

        $em = $this->getDoctrine()->getManager();
        $devis = $em->getRepository(evis::class)->find($id);
        $devis->setNsc($req->get('nsc'));
        $devis->setEmail($req->get('email'));

        $em->flush();

        $jsonContent = $Normalizer->normalize($devis, 'json', ['groups' => 'deviss']);
