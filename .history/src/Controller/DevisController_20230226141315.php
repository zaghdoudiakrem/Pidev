...
class DevisController extends AbstractController
{
    ...
    //* Dans cette fonction, nous utilisons les services NormlizeInterface et DRepository, 
    //* avec la méthode d'injection de dépendances.
    public function getDs(DevisRepository $repo, SerializerInterface $serializer)
    {
        $devis = $this->getDoctrine()->getRepository(Devis::class)->findAll();
        //* Nous utilisons la fonction normalize qui transforme le tableau d'objets 
        //* Ds en  tableau associatif simple.
        // $DsNormalises = $normalizer->normalize($Ds, 'json', ['groups' => "Ds"]);

        // //* Nous utilisons la fonction json_encode pour transformer un tableau associatif en format JSON
        // $json = json_encode($DsNormalises);
    ...
    #[Route("/D/{id}", name: "D")]
    public function DId($id, NormalizerInterface $normalizer, DRepository $repo)
    {
        $D = $repo->find($id);
        $DNormalises = $normalizer->normalize($D, 'json', ['groups' => "Ds"]);
        return new Response(json_encode($DNormalises));
    }
