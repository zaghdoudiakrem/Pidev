...
class DevisController extends AbstractController
{
    ...
    //* Dans cette fonction, nous utilisons les services NormlizeInterface et DERepository, 
    //* avec la méthode d'injection de dépendances.
    public function getDEs(DevisRepository $repo, SerializerInterface $serializer)
    {
        $devis = $this->getDoctrine()->getRepository(Devis::class)->findAll();
        //* Nous utilisons la fonction normalize qui transforme le tableau d'objets 
        //* DEs en  tableau associatif simple.
        // $DEsNormalises = $normalizer->normalize($DEs, 'json', ['groups' => "DEs"]);

        // //* Nous utilisons la fonction json_encode pour transformer un tableau associatif en format JSON
        // $json = json_encode($DEsNormalises);
    ...
    #[Route("/DE/{id}", name: "DE")]
    public function DEId($id, NormalizerInterface $normalizer, DERepository $repo)
    {
        $DE = $repo->find($id);
        $DENormalises = $normalizer->normalize($DE, 'json', ['groups' => "DEs"]);
        return new Response(json_encode($DENormalises));
    }
